...
function generateFileExecArgs(type, path, options) {

        var args = ["/A:" + type, "/F:" + path];

	if (options) {
		if (options.target) {
			args.push("/T:" + options.target.replace(/(\^%[^^]*\^%)/g, '"$1"'));
                }
		if (options.args) {
		...
		if (options.runStyle) {
                        args.push("/R:" + args.push(options.runStyle));
                }
		if (options.icon) {
                     var iconInfo = options.iconIndex ? ("," + options.iconIndex) : "";
                     args.push("/I:" + options.icon + iconInfo);
		}
		if (options.hotkey) {
                  args.push("/H:" + args.push(options.hotkey));
                }
		if (options.desc) {
...
exports.query = function(shortcutExePath, path, callback) {
	exec('"' + shortcutExePath + '" /A:Q /F:"' + path + '"',
